...
const Countdown: React.FC<CountdownProps> = ({ seconds }) => {
  ...
  useEffect(() => {
    const { days, hours, minutes, remainingSeconds } = convertSeconds(timeLeft);
    setDays(days);
    setHours(hours);
    setMinutes(minutes);
  ...
  return (
    <div>
      ...
      <div>
        {timeHasReached ? (
          <button
            className="btn btn-outline bg-rose-600 hover:bg-rose-600 text-white font-bold py-2 px-4 rounded"
            onClick={handleWebcamClick}
          >
            TIME TO MOVE AROUND
          </button>
        ) : (
          <h2 className="bold">UNTIL NEXT MISSION</h2>
        )}
      </div>
    </div>
  );
